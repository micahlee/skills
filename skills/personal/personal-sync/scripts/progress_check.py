#!/usr/bin/env python3
"""Deterministic Personal Sync progress-check helper."""
...
DEFAULT_CONFIG_PATH = Path("~/.config/agent-skills/personal-sync.json").expanduser()
LEGACY_PROGRESS_CONFIG_PATH = Path("~/.config/agent-skills/progress-check.json").expanduser()
LEGACY_DAILY_CONFIG_PATH = Path("~/.config/agent-skills/daily-note.json").expanduser()
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if path == DEFAULT_CONFIG_PATH:
        daily = read_json_object(LEGACY_DAILY_CONFIG_PATH)
        progress = read_json_object(LEGACY_PROGRESS_CONFIG_PATH)
        merged = dict(daily)
...
def resolve_daily_note_path(config: dict[str, Any], local_date: str) -> Path:
    ...
    planning_note_cli = Path(string_value(config.get("planning_note_cli"))).expanduser()
    if planning_note_cli.is_file():
        command = ["node", str(planning_note_cli), "resolve", local_date, "--vault", str(vault_path)]
        completed = subprocess.run(command, check=False, text=True, capture_output=True)
        if completed.returncode == 0:
            try:
                result = json.loads(completed.stdout)
                resolved = result.get("block", {}).get("path")
            except (json.JSONDecodeError, AttributeError):
                resolved = None
            if isinstance(resolved, str) and resolved:
                return vault_path / resolved
