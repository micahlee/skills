...
Run `python3 scripts/onboard.py` from this skill directory to collect Basecamp account/project/todoset IDs, known leaders, and any fixed assignee rules. The script writes `~/.config/agent-skills/create-monthly-worship-plan.json`. Read that file before replacing placeholders such as `<basecamp_account_id>`, `<basecamp_project_id>`, and `<core_class_assignee_id>`.
