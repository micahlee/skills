#!/usr/bin/env python3
"""End-to-end sync for the plantoeat-suggestions skill.

Pipeline:
  1. Load (or initialize) ~/.plantoeat/suggestions/state.json.
...
STATE_DIR = os.path.expanduser("~/.plantoeat/suggestions")
...
def main() -> int:
    ...
    if not state:
        ...
        doc_id = os.environ.get("PLANTOEAT_SUGGESTIONS_DOC_ID")
        if not doc_id:
            return 2
        state = {
            "doc_id": doc_id,
