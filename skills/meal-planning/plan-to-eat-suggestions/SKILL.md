...
Run `python3 scripts/onboard.py` from this skill directory to collect the suggestions doc ID, state-file path, CLI names, and taste-profile notes. The script writes `~/.config/agent-skills/plan-to-eat-suggestions.json`. Read that file before assuming document, state, or preference defaults.
...
**State:** use the configured state file, for example `~/.config/agent-skills/plan-to-eat-suggestions-state.json`.
