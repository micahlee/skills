...
Generates a Google Doc of proposed tag/metadata updates for Plan to Eat recipes, grouped into named review batches. Does not write to PTE — review the doc, then tell the agent which batches to commit (or revise).

## Onboarding

Run `python3 scripts/onboard.py` from this skill directory to collect the proposals doc ID, state-file path, batch size, and CLI names. The script writes `~/.config/agent-skills/plan-to-eat-curate.json`. Read that file before assuming document or state locations.

**State:** use the configured state file, for example `~/.config/agent-skills/plan-to-eat-curate-state.json`, as the persistent cache of recipe metadata and the review doc ID.
...
3. Tell the agent what to do per batch:
   - "Commit A, B, and D."
